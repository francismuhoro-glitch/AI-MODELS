...
/* Demo connector — seeds a realistic, always-current picture of your day:
   day-job calendar, business calendar, emails, Slack, WhatsApp.
   Everything it writes is clearly marked `demo: true` with `source: 'demo'`
   (it never masquerades as gmail/outlook/slack/whatsapp) and gets a "demo-" id
   prefix so it can be purged with POST /api/demo/purge. */
...
function emails(now) {
  const H = 36e5;
  const mk = (hAgo, fromName, from, subject, body, unread = true) => ({
    id: `demo-em-${subject.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 40)}-${Math.floor(hAgo)}`,
    source: 'demo', demo: true, from, fromName, subject, body, snippet: body.slice(0, 200),
    receivedAt: now - hAgo * H, read: !unread, labels: []
  });
  return [
    mk(2.5, 'Sarah Kimani', 'sarah.kimani@client.co.ke', 'URGENT: revised quotation needed before Friday',
    ...
    mk(5, 'Kamau Supplies Ltd', 'accounts@kamausupplies.co.ke', 'Invoice #KS-2841 — payment due in 7 days',
    ...
    mk(9, 'Grace Njeri', 'grace.n@dayjob.com', 'Prep for our 1:1 — agenda',
    ...
    mk(14, 'Jumia Sellers', 'no-reply@jumia.co.ke', 'Your weekly seller performance report',
    ...
    mk(20, 'IT Helpdesk', 'helpdesk@dayjob.com', 'Action required: password expires in 3 days',
    ...
    mk(26, 'Daniel Otieno', 'daniel.otieno@partner.co.ke', 'Partnership follow-up — our conversation last week',
    ...
    mk(30, 'Bank: Equity', 'alerts@equitybank.co.ke', 'Transaction alert: KES 85,000 received',
    ...
    mk(34, 'HR People Team', 'hr@dayjob.com', 'Reminder: complete benefits enrollment by Sept 5',
...
function messages(now) {
  const H = 36e5;
  const mk = (hAgo, channel, from, text, unread = true) => ({
    id: `demo-msg-${Math.floor(hAgo * 4)}-${channel.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`,
    source: 'demo', demo: true, channel, from, text, sentAt: now - hAgo * H, read: !unread
  });
  return [
    mk(1, 'Business — Suppliers KE', 'Wanjiku (Kikomba)', 'Good morning. The bales you wanted arrived this morning. Quality is A-grade. Should I reserve 10 for you? Going fast 🏃🏽'),
    mk(1.5, '#eng-platform', 'Brian M.', 'Deploy for the payments fix is staged. Can you approve the pipeline when you get a chance? Staging looks green.'),
    mk(2, 'Business — Customers', 'Mercy W.', 'Hello, has my order for 20 pieces been dispatched? I need them before Saturday market day.'),
    mk(3, '#general', 'Grace N.', 'Reminder: leadership sync moved to 2pm today, same link. Please review the Q3 doc before we meet.'),
    mk(4, 'Family', 'Mum', 'My son, hope week is going well. We thank God. Call me when you are free today 🙏'),
    mk(6, '#eng-platform', 'CI Bot', 'Pipeline #4213 FAILED on integration tests (3 failures). First failure: checkout-flow.spec.ts:88 — assertion on cart total.'),
    mk(8, 'Business — Suppliers KE', 'Kamau Supplies', 'Bro, the invoice balance of 46,500 — we can do a part payment if cash is tight this month. Just keep the business growing 💪'),
    mk(10, 'DM — Saruni', 'Saruni L.', 'Nice demo prep! One thing — the client asked last time about offline mode. Should we add a slide for it?'),
    mk(12, 'Business — Customers', 'Otieno K.', 'Mpesa imetumwa. KES 12,000 for the last batch. Asante sana. Next order next week.')
  ];
}
...
function sync(ctx, now = Date.now()) {
  ...
  for (const ev of evs) db.upsert('events', ev);
  const ems = emails(now);
  for (const em of ems) db.upsert('emails', em);
  const mss = messages(now);
  for (const ms of mss) db.upsert('messages', ms);
  return { events: evs.length, emails: ems.length, messages: mss.length };
}
